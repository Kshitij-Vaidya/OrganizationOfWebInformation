...
We use the Mean Reciprocal Rank (MRR) strategy to select heads. We score each (layer, head) pair based on its consistency in ranking the gold tool. Tools ranked lower have lower reciprocal score hence if the gold tool is ranked as the best consistently, that (layer, head) pair would have a score closer to 1. This way the top $K$ heads are identified (20 in this case). The following heads were selected:
\newline
...
We see that choosing only the top 20 heads performs much better than aggregating over all heads. Hence we see that relevant information is mostly present in the top k heads. This approach performs similarly to msmacro-MiniLM from part 1. 
